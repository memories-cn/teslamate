...
classDiagram
    class TeslaMate_Api {
        ...
        +list_vehicles(name) string
        +get_vehicle_with_state(name, id) string
        +stream(name, vid, receiver) string
        +signed_in?(name) boolean()
        +sign_in(name, credentials) string
        +sign_out(name) string
        -init(opts) GenServer.on_init()
        -handle_call(msg, from, state) GenServer.on_call()
        -handle_info(msg, state) GenServer.on_info()
        -refresh_tokens(tokens) string
        -schedule_refresh(auth, state) string
        -handle_result(result, auth, name) string
    }
    class TeslaApi_Auth {
        +token string
    ...
    TeslaMate_Api --> TeslaApi_Auth : "使用"
```
